...
def clean_up(texts):
    for index, text in enumerate(texts):
        ...
        text = re.sub("[^a-zA-ZàâäèéêëîïôœùûüÿçÀÂÄÈÉÊËÎÏÔŒÙÛÜŸÇεντιυχασγ]+", ' ', text)
        # text = re.sub("\u2063", '', text)
...
def run_lda(dataframe, from_date: str, to_date: str, visualize=False):
    ...
    # df = df[["Text", "Language"]]
    # for testing purposes, because i'm so tired of translating the sample each and every time:
    ...
    # language_series = df_sample["language"]
    # text_series = df_sample["Text"]
    # language_series = df_sample["Language"]

    data_list = text_series.tolist()
    # language_list = language_series.tolist()

    # translate(data_list, language_list)
    ...
    # LDA: find best amount of topics
    start, limit = [3, 25]
    ...
    # use Wikidata Api to determine topics
    ...
    for i, row in enumerate(model[corp]):
        ...
        for j, (topic_n, prop_topic) in enumerate(row):
            if j == 0:
                if data_list[i]:
                    # temp_df = pd.concat(
                    #     [temp_df, pd.Series([topic_keywords[topic_n], data_list[i], topics[topic_n], round(prop_topic, 4)])],
                    #     axis=1)
                    temp_df = pd.concat(
                        [temp_df, pd.Series(
...
if __name__ == '__main__':
    ...
    # df = csv_rw.read(FILENAME)

    run_lda(df, '2020-01-01 00:00:00', '2020-02-01 00:00:00', visualize=True)
